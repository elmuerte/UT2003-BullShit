...
class BullShitSpectator extends MessagingSpectator config(BullShit) parseconfig;
...
function bool MaySpeak(float frequency)
{
...
function string formatMessage(coerce string message, PlayerReplicationInfo Killer,
    PlayerReplicationInfo Killed, optional string kweapon, optional string vweapon)
{
    if (Killer != none)
    {
...
function NotifyKilled(Controller Killer, Controller Killed, pawn Other)
{
    ...
    if ((Killer.PlayerReplicationInfo != none) && (Killed.PlayerReplicationInfo != none))
    {
        ...
        {
            if (mayspeak(config.fKillFrequency)) DoSpeak(Killer, sayKilled(killer, killed));
        }
        if (Killed.PlayerReplicationInfo.bBot)
        {
            if (mayspeak(config.fKillFrequency)) DoSpeak(Killed, sayGotKilled(killer, killed));
        }
    }
}
        ...
        if (MaskedCompare(S, msgHelloTrigger[i]))
        {
            if (Level.TimeSeconds - LastHello >= config.iGreetDelay)
            {
                for ( C=Level.ControllerList; C!=None; C=C.NextController )
                {
                    ...
                    {
                        if (mayspeak(config.fChatFrequency))
                        {
  ...
  //log(message@switch);
    ...
    if (bFilteredMessage)
    {
        for ( C=Level.ControllerList; C!=None; C=C.NextController )
        {
            ...
            {
                if (mayspeak(config.fScoreFrequency))
                {
...
function ClientGameEnded()
{
    ...
    for ( C=Level.ControllerList; C!=None; C=C.NextController )
    {
        ...
        {
            if (mayspeak(config.fEndFrequency)) DoSpeak(C, sayGameEnd(C.PlayerReplicationInfo));
        }
    }
}
